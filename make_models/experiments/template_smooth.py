...
centre_z = Nw // 2
centre_x = int(np.floor((OVERLAP / L) * Nx))
split = Nx * Nw // 2
...
if OVERLAP >= (L / Nx) and OVERLAP <= L:
    v_orig = p.mesh_dict["V_PL"].copy()
    
    v_0 = v_orig.reshape((Nw,Nx)).T
    v_0_1 = v_0[:centre_x]
    v_0_2 = v_0[centre_x:]
    
    # v_0_1[0] = [0] * len(v_0_1[0])
    v_0_2[-1] = [0] * len(v_0_1[-1])
    
    v_0_1_s = np.linspace(v_0_1[0], v_0_1[-1], num=len(v_0_1))
    v_0_2_s = np.linspace(v_0_2[0], v_0_2[-1], num=len(v_0_2))
    
    v_0_s = v_0.copy()
    v_0_s[:centre_x] = v_0_1_s
    v_0_s[centre_x:] = v_0_2_s
    
    v_1 = v_orig.reshape((Nw,Nx))
    v_1_1 = v_1[:centre_z]
    v_1_2 = v_1[centre_z:]
    
    v_1_1_1 = v_1_1[:centre_z//2]
    v_1_1_2 = v_1_1[centre_z//2:]
    v_1_2_1 = v_1_2[:centre_z//2]
    v_1_2_2 = v_1_2[centre_z//2:]
    
    v_1_1_1_s = np.linspace(v_1_1_1[0], v_1_1_1[-1], num=len(v_1_1_1))
    v_1_1_2_s = np.linspace(v_1_1_2[0], v_1_1_2[-1], num=len(v_1_1_2))
    v_1_2_1_s = np.linspace(v_1_2_1[0], v_1_2_1[-1], num=len(v_1_2_1))
    v_1_2_2_s = np.linspace(v_1_2_2[0], v_1_2_2[-1], num=len(v_1_2_2))
    
    v_1_s = v_1.copy()
    v_1_1[:centre_z//2] = v_1_1_1_s
    v_1_1[centre_z//2:] = v_1_1_2_s
    v_1_2[:centre_z//2] = v_1_2_1_s
    v_1_2[centre_z//2:] = v_1_2_2_s
    v_1_s[:centre_z] = v_1_1
    v_1_s[centre_z:] = v_1_2
    
    p.mesh_dict["V_PL"] = np.minimum(v_1_s.ravel(),v_0_s.T.ravel())

elif OVERLAP <= (L / Nx) and OVERLAP != 0:
    print("Resolution insufficient to construct a gradient. Increase resolution, or increase spacing")

elif OVERLAP >= L:
    print("No overlap. Velocity remains constant") 

flip_second = p.mesh_dict["V_PL"].copy()
flip_second[split:] = np.flip(flip_second[split:])

p.mesh_dict["V_PL"] = flip_second
