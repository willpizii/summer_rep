...
KEY = [int(sys.argv[1]),int(sys.argv[2]),int(sys.argv[3]) * np.sin(np.radians(60))]
...
p.mesh_dict["B"][:] = 0.014
# Ensure split is defined before using it
Nw = p.set_dict["NW"]
Nx = p.set_dict["NX"]
split = (Nw * Nx) // 2

# Extract and scale the mesh coordinates
x = p.mesh_dict["X"][:split] * 1e-3
z = p.mesh_dict["Z"][:split] * 1e-3

x1 = p.mesh_dict["X"][split:] * 1e-3
z1 = p.mesh_dict["Z"][split:] * 1e-3

# Define the scale
B = p.set_dict["SET_DICT_RSF"]["B"]

zmid = z.max()-(z.max() - z.min())/2
print(zmid)

# Calculate A_part1 and A_part2 using the scaled values
A_part1 = 2 * B * (1 - 0.9 * np.exp(- ((x-2.5)**2 + ((z-zmid)*5/3)**2) / (4))).flatten()
A_part2 = 2 * B * (1 - 0.9 * np.exp(- ((x1-(2.5+KEY[0]/1000))**2 + ((z1-zmid)*5/3)**2) / (4))).flatten()

# Ensure the slices have the correct shapes before assignment
if A_part1.shape[0] == p.mesh_dict["A"][:split].shape[0] and A_part2.shape[0] == p.mesh_dict["A"][split:].shape[0]:
    p.mesh_dict["A"][:split] = A_part1
    p.mesh_dict["A"][split:] = A_part2
else:
...
# write the python object (dict) tworkingo pickle file
pickle.dump(dict2,f)
